...
import gallery from '../../Assets/lake-mountains.jpg'
    ...
    useEffect(() => {
        ...
        const fetchCarouselData = async () => {
            try {
                ...
                const response = await axios.get(`https://localhost:7026/UserSide/GetPackage?locationId=${locationId}`);
                    ...
                    <MDBRow className='row-cols-1 row-cols-md-3 g-4'>
                        {packagesdesc.map((packageData, index) => (
                            <MDBCol key={packageData.id}>
                              {/* Assuming packageData contains locationId and packageId */}
                                <Link to={`/location/package/${locationId}/dayschedule/${packageData.packageId}`}>
